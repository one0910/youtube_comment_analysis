...
from analyses.providers.ai_report import (
    AIReport, DisplayNameActivity, RepeatedTextGroup, ReportSample, ReportVideo,
)
...
'''
create_validation_criteria目的是將影片資訊、所有留言做一個整理及正規化，除了原本的留言及影片資訊外、
...
def create_validation_criteria(
    analysis_request: AIAnalysisRequest, video: ReportVideo, *, sort_order: str, include_replies: bool,
) -> PreparedReportFacts:

    #輸入須已去重；遇到無效資料拒絕，不默默刪除留言或修猜數值。
    if not isinstance(analysis_request, AIAnalysisRequest) or not isinstance(video, ReportVideo):
    ...
    sample = ReportSample(
        analysis_request.comment_count, analysis_request.main_comment_count,
...
def _build_repeated_groups(comments: tuple[AICommentInput, ...]) -> tuple[RepeatedTextGroup, ...]:
...
def _build_activity(comments: tuple[AICommentInput, ...]) -> tuple[tuple[DisplayNameActivity, ...], tuple[str, ...]]:
    ...
    activity = tuple(
        DisplayNameActivity(name, root, tuple(ids))
...
def validate_report_source_facts(report: AIReport, facts: PreparedReportFacts) -> None:
...
def replace_report_comment_refs_with_author_names(report: AIReport,facts: PreparedReportFacts) -> AIReport:
    reference_map = {
...
def apply_report_sample_scope(report: AIReport) -> AIReport:
