...
def build_report_test_fixture(*, small: bool = False):
    ...
    comments = tuple(
        AICommentInput(
            ...
            comment_text=text,
            like_count=likes[i] if i < 6 else i % 7,
            published_time_text="1 天前（模擬）",
            is_pinned=False,
        )
    ...
    payload = {
        ...
        "topics": [
            {"name": "效率提升與人工查證之間的分工", "summary": "觀眾肯定工具協助，但提醒不可省略核對。",
             "reasoning": "留言將『整理資料』與『判斷結果』分開看待。正向回饋並非完全信任 AI，而是認同工具能協助重複工作，同時保留人的核對責任。", "evidence_comment_refs": ["c1", "c2"]},
            {"name": "從概念理解到實際操作，仍需要更多案例", "summary": "具體的重試與錯誤說明，是後續內容可以補足的方向。",
             "reasoning": "這類留言提出明確的學習需求，不能直接視為否定影片品質。提供可重現的失敗案例，可能有助於觀眾理解流程的限制。", "evidence_comment_refs": ["c3"] if small else ["c3", "c9", "c11"]},
        ],
        "top_liked_comments": [
